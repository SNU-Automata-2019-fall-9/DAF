...
int* idxSortedData = NULL; //idxSortedData[l] contains the first index in sortedData such that labelData[sortedData[index]] is l
...
//construct dag data structure
    ...
    char* checked = new char[numQueryNode];
    memset(checked, 0, sizeof(char) * numQueryNode);

    int* degree=new int[numQueryNode];
    for(int i=0;i<numQueryNode;i++)
	{
		degree[i]=degreeQuery[i];
	}
    ...
    for(int i=adjIndexQuery[curr];i<adjIndexQuery[curr+1];i++){
	child=adjListQuery[i];
...
while(true)
{
	maxi=-1;
	for(int i=0;i<numQueryNode;i++){
		if(visited[i]==0&&checked[i]==1)
		{
			if(maxi==-1||degree[maxi]<degree[i])maxi=i;
		}
	}
	if(maxi==-1)break;
	curr=maxi;
